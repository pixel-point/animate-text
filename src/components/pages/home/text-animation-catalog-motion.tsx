...
import { startTextAnimationMotionLoop } from '@/lib/text-animation-motion-runtime';
import { cn } from '@/lib/utils';

export default function TextAnimationCatalogMotion() {
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 [&>*]:border-border dark:[&>*]:border-border/60 [&>*+*]:border-t xl:[&>*:nth-child(-n+2)]:border-t-0 xl:[&>*:nth-child(2n)]:border-l">
      {textAnimationCatalog.map((item) => (
        <TextAnimationCardMotion key={item.id} item={item} />
      ))}
    </div>
  );
}

function TextAnimationCardMotion({ item }: { item: TextAnimationCatalogItem }) {
  ...
  return (
    ...
    >
      ...
      <TextAnimationStageMotion item={item} />
    </article>
  );
}

function TextAnimationStageMotion({ item }: { item: TextAnimationCatalogItem }) {
  ...
  useEffect(() => {
    ...
    return startTextAnimationMotionLoop(stage, item, (error) => {
      console.error(`Failed to run Motion text animation "${item.id}"`, error);
      setFailed(true);
    });
  }, [item]);
